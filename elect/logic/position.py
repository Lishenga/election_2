...
from elect.models import position, election, candidate, voter
import datetime
...
def get_all_candidates_positions(request):  
    ...
    try:
        ...
        candidates = candidate.objects.filter(position_id=request.data['position_id'])
        page = request.GET.get('page', request.data['page'])
...
#Get positions that a voter can vote for
@api_view(['POST'])  
def get_all_positions_voter_votes_for(request):
    ...
    try:
        if request.method == 'GET':
            ...
            if election.objects.filter(status = 1).exists():
                voter_id = request.data['voter_id']
                voters = voter.objects.get(id=voter_id)
                candidates = candidate.objects.filter(region_id=voters.region_id)
                details=[]
                deta=[]
                for candis in candidates:
                    if candis.status == 'SUPER' or candis.status == '1':
                        values={
                            'id':candis.id,
                            'position_id': candis.position_id,
                            'fname': candis.fname,
                            'lname':candis.lname
                        }
                        details.append(values)
                ...
                for cats in details:
                    positions = position.objects.filter(election_id=request.data['election_id']).get(id=cats['position_id'])
                    val={
                        'id':positions.id,
                        'position_name':positions.name,
                        'position_description':positions.description,
                        'candidates_name':cats['fname'],
                        'candidates_lname':cats['lname'],
                ...
                data={'message':'Election Ended','status_code':401}
